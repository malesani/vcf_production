...
class authManager
{
    ...
    public function generateUniqueUID($tableName, $columnName, $UIdLength = 8)
    {
        ...
        // 1) Valida nomi tabella/colonna (solo lettere, numeri, underscore)
        $isValidIdent = static function (string $s): bool {
            return (bool)preg_match('/^[A-Za-z0-9_]+$/', $s);
        };
        if (!$isValidIdent($tableName) || !$isValidIdent($columnName)) {
            throw new Exception("Invalid identifier");
        }

        // 2) Verifica esistenza tabella (no SHOW; si information_schema)
        $stmt = $conn->prepare("
            SELECT 1
        ...
        // 4) Genera UID e controlla unicità
        $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
        $max = strlen($characters) - 1;

        // Prepara la query di verifica con identificatori backtickati
        $sqlCheck = sprintf(
