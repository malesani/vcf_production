    ...
    switch ($method) {
        case 'GET':
            switch ($requestData['opt']) {
                ...
                case 'nfo_reports_list':
                    ...
                    $result  = $listObj->get_nfoList(
                        [
                            ...
                            'year'           => $_GET['year']          ?? '',
                            'scheduled_from' => $_GET['scheduled_from']?? '',
                            'scheduled_to'   => $_GET['scheduled_to']  ?? '',
                            'valid_only'     => filter_var($_GET['valid_only'] ?? false, FILTER_VALIDATE_BOOLEAN),
                        ],
                        true
                    );
                ...
                // EXTRA utility (facoltative ma utili)
                case 'nfo_lastValidReport':
                    ...
                    $max_month = isset($_GET['max_month']) ? (int)$_GET['max_month'] : null;

                    $listObj = new nfoObjList($authManager, $permsManager);
                    $res     = $listObj->get_lastValidReport($requestData['managed_uid'], $year, $max_month);
                ...
                case 'nfo_validAlerts_afterLastReport':
                    if (empty($requestData['managed_uid'])) {
                        http_response_code(400);
                        echo json_encode($reqResp->toArray(
                            success: false,
                ...
                case 'nfo_validAlerts_afterLastReport_paginated':
                    if (empty($requestData['managed_uid'])) {
                        http_response_code(400);
                        echo json_encode($reqResp->toArray(
                            success: false,
                    ...
                    $listObj = new nfoObjList($authManager, $permsManager);
                    $res     = $listObj->get_allValidAlerts($requestData['managed_uid'], false, $page, $perPage);
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                ...
                case 'nfo_assets_report':
                    // Upsert asset per REPORT
                    if (empty($requestData['nfo_uid']) || empty($requestData['assets'])) {
                        http_response_code(400);
                        echo json_encode($reqResp->toArray(
                            success: false,
                            message: $trad->lang('nfo.400.missingRequiredParameters'),
                            error: 'Missing: "nfo_uid" or "assets"'
                        ));
                        break;
                    }
                    ...
                    $obj = new nfoObj($authManager, $permsManager, $requestData['nfo_uid']);
                    $res = $obj->upsert_reportAssets($requestData['nfo_uid'], $requestData['assets'], $replace);
                ...
                case 'nfo_assets_alert':
                    // Upsert asset per ALERT
                    if (empty($requestData['nfo_uid']) || empty($requestData['assets'])) {
                        http_response_code(400);
                        echo json_encode($reqResp->toArray(
                            success: false,
                    ...
                    $obj  = new nfoObj($authManager, $permsManager, $requestData['nfo_uid']);
                    $info = $obj->get_nfoData();
                    if (empty($info['nfo_uid'])) {
                        http_response_code(404);
                        echo json_encode($reqResp->toArray(
                            success: false,
                            message: $trad->lang('nfo.404.notFound'),
                            error: 'NFO not found'
                        ));
                        break;
                    }
                    if (($info['type'] ?? '') !== 'alert') {
                        ...
                        echo json_encode($reqResp->toArray(
                            success: false,
                            message: $trad->lang('nfo.400.invalidTypeForAssets'),
                            error: 'This NFO is not an alert'
                        ));
                        break;
                    }

                    $managed_uid = $info['managed_uid'] ?? '';
                    $res = $obj->upsert_alertAssets($requestData['nfo_uid'], $managed_uid, $requestData['assets'], $replace);
        ...
        case 'POST':
            switch ($requestData['opt']) {
                ...
                case 'nfo_upsert':
                    // upsert generico: crea se manca, altrimenti aggiorna info e (se presenti) assets
                    $obj = new nfoObj(
                        $authManager,
                        $permsManager,
                        $requestData['nfo_uid'] ?? null
                    );
                    $replaceAssets = filter_var($requestData['replace'] ?? true, FILTER_VALIDATE_BOOLEAN);
                    $res = $obj->upsert_nfo($requestData, $replaceAssets);
