    ...
    switch ($method) {
        ...
        case 'GET':
            switch ($requestData['opt']) {
                ...
                case 'portfolio_alignment_operation':
                    // Restituisce operazioni suggerite da report + alerts (per managed)
                    $result = $opsObj->get_portfolioAlignmentOperation();
                    http_response_code($result['success'] ? 200 : 400);
                    echo json_encode($reqResp->toArray(
                        success: $result['success'],
                        message: $trad->lang($result['message']),
                ...
                case 'valid_report_weighing':
                    // Restituisce operazioni suggerite da report + alerts (per managed)
                    $result = $opsObj->get_validReportWeighing();
                    http_response_code($result['success'] ? 200 : 400);
                    echo json_encode($reqResp->toArray(
                        success: $result['success'],
                        message: $trad->lang($result['message']),
                ...
                case 'valid_alerts_weighing':
                    // Restituisce operazioni suggerite da report + alerts (per managed)
                    $result = $opsObj->get_validAlertsWeighing();
                    http_response_code($result['success'] ? 200 : 400);
                    echo json_encode($reqResp->toArray(
                        success: $result['success'],
                        message: $trad->lang($result['message']),
                ...
                case 'valid_portfolio_weighing':
                    // Restituisce operazioni suggerite da report + alerts (per managed)
                    $result = $opsObj->get_portfolioWeighing();
                    http_response_code($result['success'] ? 200 : 400);
                    echo json_encode($reqResp->toArray(
                        success: $result['success'],
                        message: $trad->lang($result['message']),
            ...
            switch ($requestData['opt']) {
                case 'operation_create':
                    ...
                    $payload = [
                        ...
                        'nfo_uid'      => $requestData['nfo_uid']      ?? null,
                    ];
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                case 'operation_update':
                    ...
                    $res = $opsObj->update_operation($requestData['operation_uid'], $updateData);
