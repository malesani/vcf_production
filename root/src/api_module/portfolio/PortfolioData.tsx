...
// assets: symbol, value_lastOp, unitaryPrice_lastOp, value_now, unitaryPrice_now
// -----------------------------
...
function parseStatusFromBackend(isDeleted?: boolean): 'active' | 'deleted' {
...
function parseAssetsFromBackend(rawAssets: any[] | undefined, portfolio_uid: string): PortfolioAssets[] {
  ...
  return rawAssets.map((a) => {
    const unitaryPrice_lastOp = Number(a.unitaryPrice_lastOp ?? 0) || 0;
    const value_lastOp = a.value_lastOp != null ? Number(a.value_lastOp) : undefined;

    // Provo a derivare la quantità se non esiste: qty = value_lastOp / unitaryPrice_lastOp
    const unitQuantity =
      value_lastOp != null && unitaryPrice_lastOp > 0
        ? Number((value_lastOp / unitaryPrice_lastOp).toFixed(6))
        : Number(a.unitQuantity ?? 0);
...
function parsePortfolioFromBackend(raw: any): PortfolioInfo {
  // il backend restituisce direttamente i campi info + assets
  ...
  const base: PortfolioInfo = {
    ...
    // temp, verrà riassegnato poco sotto
    assets: [],
    // type/managed_uid: assegnati sotto
    ...(raw.type === 'managed'
...
function serializeCreatePayload(p: Omit<PortfolioInfo, 'portfolio_uid' | 'created_at' | 'status'> & { status?: 'active' | 'deleted' }) {
  // NB: il backend si aspetta i nomi originali:
  // - isDeleted gestito lato server (default 0). "status" non lo inviamo.
  // - type 'custom'|'managed' e, se managed, managed_uid obbligatorio.
  const payload: any = {
    user_uid: p.user_uid,
    title: p.title,
    type: p.type,
  ...
  // Assets: backend vuole: symbol, value_lastOp, unitaryPrice_lastOp
  if (Array.isArray(p.assets)) {
    payload.assets = p.assets.map((a) => {
      const unitaryPrice_lastOp = Number(a.unitaryPrice_lastOp ?? 0) || 0;
      const qty = Number(a.unitQuantity ?? 0) || 0;
      const value_lastOp = unitaryPrice_lastOp > 0 ? qty * unitaryPrice_lastOp : 0;
      return {
        symbol: a.symbol,
        value_lastOp,
        unitaryPrice_lastOp,
...
export async function get_assetPrices(
  ...
  if (response.success && response.data?.prices) {
    return { response, data: response.data.prices as AssetPrice[] };
  }
  return { response };
}
...
export async function get_portfoliosList<T = PortfolioInfo>(
  filters: Partial<Record<'search' | 'type' | 'user_uid' | 'managed_uid' | 'ranked_only' | 'created_from' | 'created_to' | 'updated_from' | 'updated_to', string>>
...
// -----------------------------
export async function get_portfoliosListPaginated(
  params: TableFilters<PortfolioInfo>
): Promise<TableDataResponse<PortfolioInfo>> {
  const response = await requestFunction(
    '/portfolios/api/portfolio.php',
    'GET',
