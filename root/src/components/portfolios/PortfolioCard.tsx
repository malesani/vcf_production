...
import { ExecutedOperationSelect } from "../../api_module/operations/OperationsRequest";
...
const PortfolioCard: React.FC<Props> = ({ portfolio, managedInfo, assetPrices }) => {
  ...
  // Valori calcolati
  const { totalInvested, totalAssetsNow, totalNow, } = useMemo(() => {
    let invested = 0;
    let now = 0;
        ...
        <div key={a.symbol} className="mb-3">
          <div className="d-flex justify-content-between small">
            <div className='d-flex align-items-center'>
              ...
              <span className="ms-2 text-muted mx-2">
                ({qty} asset)
              </span>
              <MDBBtn
                size="sm"
      ...
      <MDBCard className="rounded bg-light border shadow-sm h-100 position-relative">
        <MDBCardHeader className="py-3 px-4 border-bottom" style={{ backgroundColor: "rgb(38, 53, 80)", color: "white" }}>
          <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
            <MDBCardTitle tag="h5" className="mb-2 mb-md-0 d-flex align-items-center">
              ...
              {title}
              {(type == 'managed' && managedInfo?.title) && <MDBBadge color={'light'} className="text-dark ms-3">
        ...
        <MDBCardBody className="bg-white">
          ...
          <div className="mb-3">
            ...
            {assets.length > 0 ? renderAssetRows() : <p className="s-muted small mb-0">Nessun asset presente.</p>}
          </div>
