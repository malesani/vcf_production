            ...
            switch ($requestData['opt']) {
                case 'portfolio_data':
                    ...
                    try {
                        ...
                        $info = $obj->get_portfolioData();
                        http_response_code(200);
                        echo json_encode($reqResp->toArray(
                            success: true,
                            message: $trad->lang('portfolio.200.infoFetched'),
                            error: null,
                            data: $info
                        ));
                    } catch (Throwable $e) {
                        ...
                        echo json_encode($reqResp->toArray(
                            success: false,
                            message: $trad->lang('portfolio.404.notFound'),
                            error: $e->getMessage()
                        ));
                    }
                    break;
                ...
                case 'portfolios_list':
                    ...
                    $filters = [
                        ...
                        'include_totals' => isset($_GET['include_totals']) ? (int)$_GET['include_totals'] : 1,
                    ];
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                case 'update_portfolio':
                    ...
                    $obj = new portfolioObj($authManager, $permsManager, $requestData['portfolio_uid']);
                    $res = $obj->update_portfolioInfo($newData);
        ...
        case 'POST':
            switch ($requestData['opt']) {
                case 'create_portfolio':
                    ...
                    // Dati validi per insert: usa ALL_FIELDS_INFO (senza PK) + type
                    $newData = [];
                    ...
                    $res = $obj->create_emptyPortfolio($newData);
