...
export function GeneralInput<T extends Record<string, any>>(
  ...
  const toBool = (
    v: boolean | ((fd: T) => boolean) | undefined
  ): boolean | undefined => (typeof v === "function" ? (v as (fd: T) => boolean)(formData) : v);

  const requiredBool = toBool(field.required as any) ?? false;
  const disabledBool = toBool((field as any).disabled);
  const readOnlyBool = toBool(field.readOnly as any);

  const [isValid, setValidity] = useState(!requiredBool);
  ...
  const common = {
    ...field.properties,
    required: requiredBool || undefined,
  ...
  switch (field.type) {
    ...
    case "number":
      ...
      if (defaultValue) {
        useEffect(() => {
          let computedValidation = computeValidation<T>(field, defaultValue, formData);
          let isValid = computedValidation.isValid;
          let feedbackText = computedValidation.feedbackText;

          triggerChange(null, defaultValue, isValid, feedbackText);
        }, [defaultValue])
      }
      ...
      control = (
        <MDBInput
          {...Object.fromEntries(
            Object.entries(common).filter(([k]) => k !== 'defaultValue')
          )}
          type={"number"}
          label={field_label}
          labelClass={label_class}
