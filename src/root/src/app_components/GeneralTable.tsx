...
type FetchReturn<T> = Promise<DataResponse<T[]> | requestResponse>;

interface FetchProps<T, P, F> {
    getData: (args: P & F) => FetchReturn<T>;
    response?: never;
    data?: never;
}
...
export function GeneralTable<T extends Record<string, any>, P extends Record<string, any>, F extends Record<string, any> = DefaultFilters>(props: GeneralTableProps<T, P, F>) {
    ...
    const isFetchMode =
        "getData" in props &&
        typeof (props as FetchProps<T, P, F>).getData === "function";
    ...
    let getData: ((args: P & F) => FetchReturn<T>) | undefined;
    ...
    // Inline adapter → normalizza sia DataResponse<T[]> che requestResponse/TableDataResponse<T>
    function normalizeToDataResponse<U>(
        raw: DataResponse<U[]> | requestResponse
    ): {
        response: requestResponse;
        rows?: U[];
        meta?: { items_num?: number; pages_num?: number; page?: number; per_page?: number };
    } {
        // Caso 1: già nel formato DataResponse<U[]>
        if ((raw as any)?.response) {
            const dr = raw as DataResponse<U[]>;
            // Provo a recuperare meta dal payload "response.data" se presente
            const respAny = dr.response as any;
            const meta =
                respAny?.data?.meta ??
                (respAny?.data && typeof respAny.data === 'object'
                    ? {
                        items_num: respAny.data.items_num,
                        pages_num: respAny.data.pages_num,
                        page: respAny.data.page,
                        per_page: respAny.data.per_page,
                    }
                    : undefined);
        ...
        // Caso 2: requestResponse (es. TableDataResponse<T>)
        const rr = raw as requestResponse;
        let rows: U[] | undefined;
        let meta:
            | { items_num?: number; pages_num?: number; page?: number; per_page?: number }
            | undefined;

        // a) shape: { data: { rows: U[], meta: {...} } }
        if (rr?.data && Array.isArray(rr.data.rows)) {
            rows = rr.data.rows as U[];
            meta = rr.data.meta ?? {
                items_num: rr.data.items_num,
                pages_num: rr.data.pages_num,
                page: rr.data.page,
                per_page: rr.data.per_page,
            };
        }
        // b) shape: { data: U[] }
    ...
    const fetchData = useCallback(
        async (f: F) => {
            ...
            try {
                ...
                const raw = await getData!(args)!;              // <-- può restituire entrambi i formati
                const norm = normalizeToDataResponse<T>(raw);   // <-- normalizzo

                if (norm.response.success && norm.rows) {
                    const wrapped = norm.rows.map(item => ({
                        __internalId: makeId(),
                        data: item,
                    }));
                    setRows(wrapped);
                    ...
                    // Paginazione: preferisci meta, altrimenti fallback
                    const total = Number(norm.meta?.items_num ?? wrapped.length);
                    const perPage = Number(norm.meta?.per_page ?? f.per_page);
                    setTotalPages(Math.max(1, Math.ceil(total / perPage)));
                } else {
                    showAlertError(norm.response.message || "Errore caricamento dati");
                }
            } catch (err: any) {
    ...
    const refreshData = useCallback(
        async (withLoading: boolean = true) => {
            ...
            try {
                ...
                const raw = await getData!(args)!;
                const norm = normalizeToDataResponse<T>(raw);
                ...
                if (norm.response.success && norm.rows) {
                    ...
                    const total = Number(norm.meta?.items_num ?? wrapped.length);
                    const perPage = Number(norm.meta?.per_page ?? filters.per_page);
                    setTotalPages(Math.max(1, Math.ceil(total / perPage)));
                } else {
                    showAlertError(norm.response.message || "Errore caricamento dati");
                }
